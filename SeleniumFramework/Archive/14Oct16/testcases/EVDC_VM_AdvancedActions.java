...
public class EVDC_VM_AdvancedActions {
	...
	public static String sVMAppName,sVMstatus,sVMName;
...
*/
	...
	public void EVDC_Advanced_SaveVMasTemplateErrorWithMountMedia(){
		...
		FrameworkUtil.sTestCaseName = "EVDC_Advanced_SaveVMasTemplateErrorWithMountMedia";
...
/*				if (WebDriverUtil.browser.equalsIgnoreCase("IE")){
				WebObjectUtil.scrollToElement(expandArrow);
				expandArrow.sendKeys(Keys.ENTER);
				}else{
					expandArrow.click();
				}*/
	...
	public static int getRow_EVDC_VMTable(WebElement webTable,String sSearchString,String sOSType){
		int index =0;
		int isearchIndex = 0;
		String sOS;
...
/*						sVMName = WebTableUtil.getCellData(webTable, i, 3);
						...
						*/
						...
						sVMAppName=CustomWebElementUtil.getCellData(isearchIndex, 3);
						sVMName=CustomWebElementUtil.getCellData(isearchIndex, 2);
...
/*				//if (WebObjectUtil.getToolTipsMsg(EVDC_Advanced.getNav_EVDCsaveToCatalogComboParentObject(), "Required Field")){
				...
				String sConfirmMsg = (String)FrameworkUtil.dictPropertyData.get("NAV.EVDC.SAVEASTEMPLATE.CONFIRMMSG") + " " + sVMAppName + " as template "+ sTemplateName + "?";
	...
	return bStatus ;
	 ...
	 public static boolean verifyDeleteVM(){
		 ...
		 try {			
				...
				sConfirmMsg = sConfirmMsg + " " + sVMAppName + " ?";
				...
				sSuccessMsg = sSuccessMsg + " " + sVMAppName;
				boolean bSuccessMsg = EVDC_Advanced.getNav_EVDCConfirmPopup().getText().contains(sSuccessMsg);
				FrameworkUtil.updateCustomResultBasedOnStatus(bSuccessMsg,sSuccessMsg);
				WebObjectUtil.clickElement(EVDC_Advanced.getNav_EVDCConfirmPopupOkBtn());
	 ...
	 public static boolean verifyCopyVM(){
		 ...
		 try {			
				...
				//The below four lines are updated by QAA03 on 01/25/2016
				int iRowNbr = EVDC_VM_AdvancedActions.iVisibleVMRowNum ;
				WebElement wVMNameCopy = WebObjectUtil.getWebElementFromDynamicLocator("xpath","EVDC.VM.VMTABLE.VMNAME", "PARAMETER_ROWNUMBER", iRowNbr+"");
				String sVMNameCopy = wVMNameCopy.getText();
				sConfirmMsg = sConfirmMsg + " " + sVMNameCopy + " as " + sNewVMname + " ?";
				...
				//The Below line updated by QAA03 on 01/25/2016
				//sSuccessMsg = sSuccessMsg + " " + sVMName + " as " + sNewVMname + " has been successfully submitted" ;
				sSuccessMsg = sSuccessMsg + " " + sVMNameCopy + " as " + sNewVMname + " has been successfully submitted" ;
				if (WebObjectUtil.isElementPresent(EVDC_Advanced.getNav_EVDCConfirmPopup())){
					bSuccessMsg = EVDC_Advanced.getNav_EVDCConfirmPopup().getText().contains(sSuccessMsg);
					FrameworkUtil.updateCustomResultBasedOnStatus(bSuccessMsg,sSuccessMsg);
					WebObjectUtil.clickElement(EVDC_Advanced.getNav_EVDCConfirmPopupOkBtn());
