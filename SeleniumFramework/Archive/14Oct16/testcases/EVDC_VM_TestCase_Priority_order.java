...
public class EVDC_VM_TestCase_Priority_order {
		...
		public void navEVDCVMTabListedInfo()
		{
			...
			String sActBreadCrumb;
				try {
					...
					//WebObjectUtil.scrollElementAtPageCenter(EVDC_OverviewPF.getNav_VMCountNumberLink());
					WebObjectUtil.clickElement(EVDC_OverviewPF.getNav_VMCountNumberLink());
	...
	public void navValEditComputStorageAddChkDsk() 
	{
		...
		try {
			...
			String sVMNameorStatus=(String)FrameworkUtil.dictPropertyData.get("NAV.EVDC.UNIX.VMNAME.OR.STATUS.STOPPED.EVEN");
			EVDCLoginAndExpand(sVMNameorStatus);
			iRowNbr = EVDC_VM_AdvancedActions.iVisibleVMRowNum;
			stopRunningVM(iRowNbr);
	...
	public void navValComputStorageAddDelIcon()
	{
		...
		try {
			...
			String sVMNameorStatus=(String)FrameworkUtil.dictPropertyData.get("NAV.EVDC.UNIX.VMNAME.OR.STATUS.STOPPED.ODD");
			EVDCLoginAndExpand(sVMNameorStatus);
			iRowNbr = EVDC_VM_AdvancedActions.iVisibleVMRowNum;
			stopRunningVM(iRowNbr);
			//Verifying and Clicking on Edit button
	...
	public void navValNICIPModeFunctionality()
	{
		...
		try {
			...
			else
			{
				...
				LoggerUtil.log_type_error("IP Address field is disabled for Static Manual mode" );
			}
	...
	public void navVal2AddDelNICMsg()
	{
		...
		try {
			...
			CustomWebElementUtil.loadCustomElement(EVDC_VMPF.getNav_NICWebTable(), "table");
			...
			//Commenting the above two line -- as the element has been changed - Vivek - 01192016
			...
			for(int i=2;i<=iRowCount;i++)
			{
				CustomWebElementUtil.loadCustomElement(EVDC_VMPF.getNav_NICWebTable(), "table");
				//sNicID =	WebTableUtil.getCellData(wNICTable, i, 1);
				sNicID=CustomWebElementUtil.getCellData(i, 1);
				//WebElement wRadioStatus=	WebTableUtil.getCellAsWebElement(wNICTable, i, 2);
				//boolean bRadioStatus=WebObjectUtil.isCheckRadioSelected(wRadioStatus);
				//sNICPrimary = "nicPrimary-" + sNicID;
				//wNICPrimary = WebObjectUtil.GetWebElement("id", sNICPrimary, "NIC Primary");
				// This object is updated on 10/14/2015 By QAA04
				//wNICPrimary = WebObjectUtil.getWebElementFromDynamicLocator("xpath", "EVDC.VM.VMTABLE.NICTABLE.PRIMARY", "PARAMETER_NICID", sNicID);
				wNICPrimary = WebObjectUtil.getWebElementFromDynamicLocator("xpath", "EVDC.VM.VMTABLE.NICTABLE.PRIMARY", "PARAMETER_NICID", i+"");
				bRadioStatus=WebObjectUtil.isCheckRadioSelected(wNICPrimary);
	...
	public void navValUserNameRequired()
	{
		...
		try {
			...
			if(WebObjectUtil.isElementPresent(EVDC_VMPF.getNav_NoVMEditPopUpMsg()))
			{
				WebObjectUtil.clickElement(Common.getNav_CommonPopUpOKButton());
				WebObjectUtil.clickElement(EVDC_VMPF.getNav_CloseWindowButton());
	...
	public void navValChkSaveEdtBtn()
	{
		...
		try {
			...
			if(WebObjectUtil.isElementPresent(EVDC_VMPF.getNav_NoVMEditPopUpMsg()))
			{
				WebObjectUtil.clickElement(Common.getNav_CommonPopUpOKButton());
				WebObjectUtil.clickElement(EVDC_VMPF.getNav_CloseWindowButton());
				FrameworkUtil.writeCustomResult(FrameworkUtil.stepNo(), FrameworkUtil.sElementName , "Saving EDIT VM", "done", "Saving Edit VM without update - Already checkbox is checked");
			}
			else
			{
				if(WebObjectUtil.isElementPresent(EVDC_VMPF.getNav_NoVMEditPopUpMsg()))
...
//-------///Clicking Stop Button and verifying the userfeedback msg
	...
	==============================================================*/
	@Test(priority=19)
	public void CreateTicketFromVM(){
		WebDriver driver = WebDriverUtil.getDriver();
		PageFactory.initElements(driver, EVDC_OverviewPF);	
		PageFactory.initElements(driver, EVDC_VMPF);
		...
		try {
			...
			FrameworkUtil.sTestCaseName = "EVDC-CreateTicketFromVM";
...
==============================================================*/
	...
	public void verifyNoNetworksAddVM()
	{
			...
			String sUserName=null,sPassword=null,sVMName=null,sBlankVMName,sTemplateVMName;
		...
		try {
			...
			sBlankVMName =sVMName + "_" + FrameworkUtil.sTimeStamp;
			...
			WebObjectUtil.clickAndVerifyElement(EVDC_VMPF.getNav_AddVMLink(), EVDC_VMPF.getNav_AddVMSelectVMType());
			EVDC_VM_TestCase.navAddBlankVM(sBlankVMName);
			// Clicking on Add Virtual Machine Link
			WebObjectUtil.clickAndVerifyElement(EVDC_VMPF.getNav_AddVMLink(), EVDC_VMPF.getNav_AddVMSelectVMType());
			sTemplateVMName =sVMName + "_" + FrameworkUtil.sTimeStamp;
			EVDC_VM_TestCase.navAddVMFromTemplate(sTemplateVMName);
...
==============================================================*/
	...
	public void valVMStoppedEdtSuccess()
	{
		...
		try {
			...
			sUserName = (String)FrameworkUtil.dictPropertyData.get("NAV.USERID.KIRKJ");
			sPassword = (String)FrameworkUtil.dictPropertyData.get("NAV.PWD.KIRKJ");
	...
	Release         : Hague
	Pre-Requisites	:
	Revision History:Change the UserId to QaAuto and Added two new variable with name sVMName_BlankVM, sVMName_Template;
					 Commented sEVDCOrgName as i am not seeing anyuse - 02152016
	==============================================================*/
	@Test(priority=27)
	public void VerifyVMPortlet()
	{
		...
		String sVMName_BlankVM,sVMName_Template;
		...
		//Changing to QaAuto- Viek - 02/15/2016
		...
		sVMName_BlankVM=sVMName + "_"+FrameworkUtil.sTimeStamp;
		...
		EVDC_VM_TestCase.navAddBlankVM(sVMName_BlankVM);
		
		//Creating VM from Template
	    WebObjectUtil.explicitWait(5);		
	    sVMName_Template=sVMName + "_"+FrameworkUtil.sTimeStamp;
	    
		WebObjectUtil.clickAndVerifyElement(EVDC_VMPF.getNav_AddVMLink(), EVDC_VMPF.getNav_AddVMSelectVMType());
		EVDC_VM_TestCase.navAddVMFromTemplate(sVMName_Template);
	...
	/*==============================================================
	TestScript  	: navValMultipleEdit
	Manual TestCase	: WCP_4300_TC_1.2_Edit VM-Mutiple Edits	
	Description		: To perform multiple edits
	Author 			: Vivek Kumar
	...
	==============================================================*/
	@Test(priority=114)
	public void navValMultipleEdit()
	{
		WebDriver driver = WebDriverUtil.getDriver();
		PageFactory.initElements(driver, EVDC_OverviewPF);	
		PageFactory.initElements(driver, EVDC_VMPF);
		...
		int iRowNbr;
		boolean bFlag=false;
		String sUserMessage,sTestDescription,sVMName,sActMessage,sExpMessage,sHostName;
		sUserMessage=(String)FrameworkUtil.dictPropertyData.get("NAV.EVDC.ADDDISK");
		
		 sTestDescription= "SeleniumTest_" + FrameworkUtil.sTimeStamp;
		 sHostName="SeleniumTest-" + FrameworkUtil.sHostTimeStamp;
		try {
			FrameworkUtil.sTestCaseName = "EVDC-navValMultipleEdit";
			FrameworkUtil.sManualTestCaseName="QAT-1543-WCP_4300_TC_1.2_Edit VM-Mutiple Edit";
			FrameworkUtil.updateTestCaseNameinResult(FrameworkUtil.sTestCaseName,FrameworkUtil.sManualTestCaseName);
			...
			//Verifying and Clicking on Edit button
			navVerifyClkEditBtn(iRowNbr);
			//Editing General Tab link
			WebObjectUtil.verifyElementPresent(EVDC_VMPF.getNav_EditGeneralTab());
			WebObjectUtil.clickElement(EVDC_VMPF.getNav_EditGeneralTab());
			//Editing info in Description input box
			WebObjectUtil.SetValueEdit(EVDC_VMPF.getNav_EditDescriptionInput(), sTestDescription);
			
			//Editing Compute & Storage
			WebObjectUtil.clickElement(EVDC_VMPF.getNav_EditComputeStorage());
			WebObjectUtil.waitForElementPresent(EVDC_VMPF.getNav_ComputeStorageAddDisk());
			WebObjectUtil.clickElement(EVDC_VMPF.getNav_ComputeStorageAddDisk());
			...
			sExpMessage=sVMName+sUserMessage;
			//System.out.println(sActErrorMessage);
			bFlag=sActMessage.contains(sExpMessage);
			FrameworkUtil.updateCustomResultBasedOnStatus(bFlag, "Message Verified - Actual = "+ sActMessage + " Expected message = " +sExpMessage );
...
//######################################################################################
		...
		public static void EVDCLoginAndExpand(String sVMNameorStatus)
		{
			...
			FrameworkUtil.writeCustomResult(FrameworkUtil.stepNo(), "Get VM and its State" , "VM Name and State ", "Done", EVDC_VM_AdvancedActions.sVMAppName + "//"+ EVDC_VM_AdvancedActions.sVMstatus);
		}
	...
	public static boolean stopRunningVM(int iRowNum ){
		...
		CustomWebElementUtil.loadCustomElement(EVDC_VMPF.getNav_VMWebTableParent(), "table");
		int iCurrentRowNo=CustomWebElementUtil.getRowByCellText( EVDC_VM_AdvancedActions.sVMAppName, 1);
...
// _________________________________ out of scope test cases ______________________________________________________ //

/*==============================================================
TestScript  	: verifyVMDataforPrivateEVDC
Manual TestCase	: QAT-482 - WCP_3218_TC_Private Cloud--Virtual Machines.
Description		: Verifying VM data by clicking on private EVDC
Author 			: QAA03
...
@Test(priority=31)
public void verifyVMDataforPrivateEVDC()
{
				WebDriver driver = WebDriverUtil.getDriver();
				PageFactory.initElements(driver,EVDC_OverviewPF);
				PageFactory.initElements(driver,EVDC_VMPF);
			...
			try {
				FrameworkUtil.sTestCaseName = "verifyVMDataforPrivateEVDC";
				FrameworkUtil.sManualTestCaseName="QAT-482 - WCP_3218_TC_Private Cloud--Virtual Machines.";
				FrameworkUtil.updateTestCaseNameinResult(FrameworkUtil.sTestCaseName,FrameworkUtil.sManualTestCaseName);
				...
				WebObjectUtil.waitForElementPresent(EVDC_OverviewPF.getNav_EVDCOverviewVirtualMachine());
				...
				//Verifying VM data is present or not               
                CustomWebElementUtil.loadCustomElement(EVDC_VMPF.getNav_VMWebTableParent(),"table");
                
                iVMCount = CustomWebElementUtil.iRowCount;
                
                bVMStatus = iVMCount>0;
                
                FrameworkUtil.updateCustomResultBasedOnStatus(bVMStatus, "VMs for Private EVDC");
				...
				LoggerUtil.log_type_error("catch block:" + FrameworkUtil.sElementName);
				FrameworkUtil.writeCustomResult(FrameworkUtil.stepNo(), FrameworkUtil.sElementName , "verifyVMDataforPrivateEVDC", "warn", "catch block:" +e.getMessage());
		}
	}


/*==============================================================
TestScript  	: EVDC_CreateTicketUsingEntity02
Manual TestCase	: QAT-1474-Check VM Create a ticket functionality
Description		: Create Ticker from VM table                                                                                         
...
==============================================================*/
@Test(priority=33)
public void EVDC_CreateTicketUsingEntity02(){
	WebDriver driver = WebDriverUtil.getDriver();
	PageFactory.initElements(driver, EVDC_OverviewPF);	
	PageFactory.initElements(driver, EVDC_VMPF);
	
	int iRowNo;
	WebElement wEVDC_Name;
	try {
		
		FrameworkUtil.sTestCaseName = "EVDC_CreateTicketUsingEntity02";
		FrameworkUtil.sManualTestCaseName=" QAT-1474-Check VM Create a ticket functionality";
		FrameworkUtil.updateTestCaseNameinResult(FrameworkUtil.sTestCaseName,FrameworkUtil.sManualTestCaseName);
		int iRowNbr;
		
		
		String sVMNameorStatus="Stopped";
		//String sVMNameorStatus=(String)FrameworkUtil.dictPropertyData.get("NAV.EVDC.VM.RUNNING.STATUS");
		LoginTestCase_Old1.LoginAsEntity02User();
		//WebObjectUtil.clickAndVerifyElement(EVDC_OverviewPF.getNav_EVDCname(), EVDC_OverviewPF.getNav_EVDCOverviewVirtualMachine());
		wEVDC_Name=EVDC_VM_TestCase.getEVDCNameAsWebElement("Nashville, TN - tstentity02");
		WebObjectUtil.clickAndVerifyElement(wEVDC_Name, EVDC_OverviewPF.getNav_EVDCOverviewVirtualMachine());
		WebObjectUtil.clickAndVerifyElement(EVDC_OverviewPF.getNav_EVDCOverviewVirtualMachine(),EVDC_VMPF.getNav_AddVMLink());
		...
		String sUsrFedBackMsg=EVDC_VMPF.getNav_UserfeedBack().getText();
		String[] arrFedBackMsg=sUsrFedBackMsg.split(":");
		
		sUsrFedBackMsg=arrFedBackMsg[1].trim();
		arrFedBackMsg=sUsrFedBackMsg.split(" ");
		String sTicketID=arrFedBackMsg[0];
...
/*==============================================================
TestScript  	: verifyCreateTktfrmStorageTier
Manual TestCase	: QAT-1467-WCP_3415_TC_1.7_Ticket watchers_Storage_Add storage tier to VDC ticket
Description		: Create Ticket from Storage Tier                                                                                         
...
@Test(priority=35)
public void verifyCreateTktfrmStorageTier()
{
	...
	try {
		FrameworkUtil.sTestCaseName = "verifyCreateTktfrmStorageTier";
		FrameworkUtil.sManualTestCaseName=" QAT-1467-WCP_3415_TC_1.7_Ticket watchers_Storage_Add storage tier to VDC ticket";
		FrameworkUtil.updateTestCaseNameinResult(FrameworkUtil.sTestCaseName,FrameworkUtil.sManualTestCaseName);
	...
	//	String sUserName = (String)FrameworkUtil.dictPropertyData.get("NAV.USER.11GIRAFFS.USERID.AROLE");
      ...
      WebElement wb =  WebObjectUtil.getActiveElementasWebElement((String)FrameworkUtil.dictDynamicPagePactory.get("CUSTOM.WEBLIST.XPATH"));
      WebObjectUtil.selectCustomWebList(wb,1);
      
      sAddStoreTierTitle = NavigatorCommonMethods.createTicketDialogSubmit("StorageTier");
     // sAddStoreTierTitle = sAddStoreTierTitle.substring(0, 22);
    /*  
     // WebObjectUtil.selectCustomWebList(EVDC_VMPF.getNav_StorageTierPopupDropdown(),1);
      ...
      // WebObjectUtil.SetValueEdit(EVDC_VMPF.getNav_AddStorageTierNotePopup(), sAddStoreTierNote);
       EVDC_VMPF.getNav_AddStorageTierNotePopup().sendKeys(sAddStoreTierTitle);
       WebObjectUtil.explicitWait(3);
       WebObjectUtil.clickElement(EVDC_VMPF.getNav_AddStorageTierOkBtn());*/
       WebObjectUtil.explicitWait(5);
       TicketTestCase.navigateToTicketPage();
       TicketTestCase.searchTicket("Summary",sAddStoreTierTitle);
       TicketTestCase.SearcResultFound("Summary", 3, sAddStoreTierTitle);
       LoginTestCase_Old1.nav_Logout();
	}
...
/*==============================================================
MethodName 		: waitForScreenMaskDisappeared
Manual TestCase	: 
Description		: Wait for page mask screen should be disappeared so that we can access VM page.
Author 			: Arul
Creation Date 	: 07/06/2016
release Name    : 
Pre-Requisites	:
Revision History: taken from sukur method "waitForFeedbackMsgNotdisplayed"
==============================================================*/
public static void waitForScreenMaskDisappeared()
 {
	Date d=new Date();
	  ...
	  try{
	   ...
	   {
	    WebObjectUtil.explicitWait(5);
	    if(EVDC_VMPF.getNav_MaskVMPage().isDisplayed()) 
	     WebObjectUtil.explicitWait(30);
	     endSec=endSec+d.getSeconds();
	     System.out.println("VM update wait Time elapsed : " + endSec);
	    if(endSec>180)
	     break;
	    FrameworkUtil.writeCustomResult(FrameworkUtil.stepNo(), FrameworkUtil.sElementName, "VM update in-Progress", "done", "Wait for VM update ");
	   }
	
	   while(EVDC_VMPF.getNav_MaskVMPage().isDisplayed());
	
	   FrameworkUtil.writeCustomResult(FrameworkUtil.stepNo(), FrameworkUtil.sElementName, "VM update in-Progress", "done", "VM update Total wait time: " + (endSec-dStrtsec));
	   }
	   catch(Exception e)
	   {
	    System.out.println("waitForScreenMaskDisappeared method catch block:" + e.getMessage());
	   }
