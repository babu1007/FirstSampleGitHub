...
/*    		 System.out.println("row count: " + iRowCount);
	...
	public static boolean verifyTextOnRow(WebElement webTable, int iRowNo, String sText){
		boolean bStatus = false;
		String sRowContent = null;
		WebElement wRow =  getRowAsWebElement( webTable, iRowNo);
		
		sRowContent=wRow.getText();
			if (sRowContent.contains(sText) ){
					bStatus = true;
			}
    		return bStatus;
	}
	...
	public static WebElement getChildElementFromWebTable(WebElement webTable,String sChildElementTagName,int iRowNo,int iColNo){
		WebElement wChildElement = null, wCell;
		
		wCell = getCellAsWebElement(webTable, iRowNo, iColNo);
		List<WebElement> wChildElementList =  wCell.findElements(By.tagName(sChildElementTagName));
		
		if(!wChildElementList.isEmpty()){
			for (WebElement wChild : wChildElementList){
				if (wChild.isDisplayed()){
					wChildElement=wChild;
				}
			}
		}
		return wChildElement;
	}
